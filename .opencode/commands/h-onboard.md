...
The supported v7 host-agent surfaces for this project are Claude Code, Codex,
and the checked-in OpenCode MCP configuration. Cursor, Gemini CLI, JetBrains
Air, and other MCP clients remain experimental carriers; do not assume their
tooling matches this contract.
...
5. Draft the YAML `spec-section` block in the carrier file named by
   ...
   - `target_refs` — for boundary sections, enumerate at least four CHR-10
     corners. Use either explicit role names or stable normative aliases that
     name the perspective source (for example, law/admissibility/deontics/evidence
     references in project documents). Keep the alias format specific enough for
     a later agent to resolve which corner it represents.
   - `evidence_required[].kind` — declare the guard or evidence location:
     `type | L1 | L2 | L3 | L4 | DB | E2E | manual`. `manual` is valid for
     admissibility, evidence-policy, or other statements whose satisfaction is
     an operator/review process rather than an executable check.
