...
- Read the `spec_section_refs` field returned by the resolver; pass those IDs
  as `section_refs` when calling
  `haft_decision(decide)`.
- Use repository-native file search and reads (`rg --files`, `rg`, and normal
  file reads) to ground the chosen variant in actual code paths so
  `affected_files` is concrete, not aspirational.
