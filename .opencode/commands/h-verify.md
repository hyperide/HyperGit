...
Call `haft_query(action="check")` — the CI-actionable enforcement report for
decision/evidence debt and spec health:
...
Then query the spec coverage surface (`haft spec coverage --json` in shell
mode) and include uncovered active SpecSections as Verify findings even when
`check` is clean. Use `status` for an at-a-glance overview; use `check` plus
spec coverage when the operator or CI must act on debt. If you need
finer-grained legacy data,
