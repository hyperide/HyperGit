...
This explicitly selects active DecisionRecords, generates an inspectable
`.haft/plans/*.yaml`, creates bounded WorkCommissions, and stops before
execution. A bare `haft harness run` only runs existing runnable commissions; it
does not select decisions by itself.
...
Per V3 drain invariants, carried here as external Haft runtime rules rather
than as a local HyperGit DecisionRecord:
